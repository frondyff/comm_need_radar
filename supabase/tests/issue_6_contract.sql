...
begin
    ...
    if to_regprocedure(
        ...
        raise exception 'Private scoring-contract-02 publication function is missing';
    end if;
    if to_regprocedure(
        'public.publish_scoring_contract_03(jsonb,jsonb,jsonb)'
    ) is null then
        raise exception 'Private scoring-contract-03 publication function is missing';
    end if;
    if has_function_privilege(
        ...
        raise exception 'Scoring-contract-02 publication permissions are unsafe';
    end if;
    if has_function_privilege(
        'anon',
        'public.publish_scoring_contract_03(jsonb,jsonb,jsonb)',
        ...
        raise exception 'Scoring-contract-03 publication permissions are unsafe';
    end if;
