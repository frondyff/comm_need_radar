...
| Issue | Current state |
| --- | --- |
| `#5` | Canonical `feature/dashboard` baseline and data integration are merged; product/UX acceptance remains open |
| `#6` | Schema, RLS, atomic loading, and validation are merged |
| `#7` | Supabase dashboard adapter is merged into the canonical frontend |
| `#8` | Real boundary artifact and stable-ID integration are merged; final UX review remains open |
| `#14` | Server route scaffolding is in progress; Vercel preview and UI integration are not complete |
...
- `cd frontend && npm ci`
- `cd frontend && npm run typecheck:api`
...
1. Document or add migrations for the current app-ready tables:
   `area_profile`, `gap_score`, `accessibility`, `service_table`,
...
2. Map `service_table` into dashboard fields:
   `service_name` to display name, `service_category` to display type,
   `address`, `phone`, `latitude`, `longitude`, and `website` directly.
3. Derive `langs` by splitting the `language` field.
...
- The service map and service list must query the real 4,255-row service layer
  or a bounded paginated equivalent.
...
- Run `npm ci`, `npm run typecheck:api`, `npm run build`, and
