...
async function loadPagedServices(client: any): Promise<ServiceRow[]> {
  ...
  while (true) {
    ...
    rows.push(...((data ?? []) as ServicesMasterRow[]));
  ...
  return rows.map((row) => ({
    ...
    service_category: row.primary_category,
...
export default async function handler(req: any, res: any) {
  ...
  const allServices = await loadPagedServices(client);
  ...
  const candidates = allServices
    ...
    .filter((service) => service.distance_km <= radiusKm || radiusKm <= 0)
  ...
  const prompt = {
    area: {
      ...
      structural_vulnerability_score: areaRow.structural_vulnerability_score,
