...
// Fixed template copy (title, section headers, footer) is our own text, so
// it's simply translated here — no "missing data" concern. What genuinely
// *can't* be translated is the underlying service content pulled from the
// database (service descriptions, nearby-service tags): that's flagged with
// FlyerFrenchNotice instead of silently printing English under a French flyer.
const FLYER_STRINGS = {
  title: { en: ["Community", "service support"], fr: ["Soutien aux services", "communautaires"] },
  tagline: { en: "Practical help for people in your community.", fr: "Une aide pratique pour les gens de votre communauté." },
  servicesOffered: { en: "Services offered", fr: "Services offerts" },
  alsoOnThisMap: { en: "Also on this map", fr: "Également sur cette carte" },
  call211: { en: "Call 211 for more", fr: "Composez le 211 pour plus d'infos" },
  call211Aria: { en: "Call 211 for more information", fr: "Composez le 211 pour plus d'informations" },
  contactForDetails: { en: "Contact this service for details.", fr: "Contactez ce service pour plus de détails." },
  addressNotListed: { en: "Address not listed", fr: "Adresse non répertoriée" },
};

// Single, flyer-wide disclaimer instead of repeating the same note under
// every section that happens to contain English-only DB content — placed
// right under the colored header band so it's the first thing read after
// the org's own details, before any English content appears below it.
function FlyerFrenchNotice({ isEN }) {
  if (isEN) return null;
  return <div style={{ background: "#FFFBEB", borderBottom: "1px solid #FDE68A", color: "#B45309", fontSize: 7.6, fontStyle: "italic", lineHeight: 1.3, padding: "5px 16px" }}>
    (Certaines sections de ce dépliant ne sont pas encore offertes en français — English content may appear below.)
  </div>;
}

export function FlyerPreview({ flyer, isEN = true }) {
  if (!flyer) return null;
  return <UniversalFlyerPreview flyer={flyer} isEN={isEN} />;
}

function UniversalFlyerPreview({ flyer, isEN }) {
  ...
  return <div id={FLYER_PREVIEW_ELEMENT_ID} style={flyerPreviewStyle}>
    <FlyerMap location={location} service={service} nearbyServices={nearbyServices} isEN={isEN} />
    ...
    <section style={{ background: categoryColor, color: "#fff", display: "grid", gap: 9, gridTemplateColumns: "54px minmax(0, 1fr)", padding: "10px 16px" }}>
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <FlyerContact icon={MapPin}>{getStreetAddress(service.address, isEN)}</FlyerContact>
    ...
    <FlyerFrenchNotice isEN={isEN} />

    <main style={{ padding: "9px 16px 0" }}>
      <div style={{ display: "grid", gap: 12, gridTemplateColumns: "1.08fr 0.92fr" }}>
        <section style={{ minWidth: 0 }}>
          <h1 style={{ color: FLYER_COLORS.black, fontSize: 19.5, fontWeight: 850, letterSpacing: -0.7, lineHeight: 0.98, margin: "0 0 8px" }}>{isEN ? FLYER_STRINGS.title.en[0] : FLYER_STRINGS.title.fr[0]}<br />{isEN ? FLYER_STRINGS.title.en[1] : FLYER_STRINGS.title.fr[1]}</h1>
          <p style={{ color: FLYER_COLORS.black, fontSize: 10, fontWeight: 600, lineHeight: 1.23, margin: "0 0 9px" }}>{isEN ? FLYER_STRINGS.tagline.en : FLYER_STRINGS.tagline.fr}</p>
        ...
        <section style={{ borderLeft: `1px solid ${FLYER_COLORS.border}`, minWidth: 0, paddingLeft: 13 }}>
          <h2 style={{ borderBottom: `2px solid ${categoryColor}`, color: categoryColor, fontSize: 13, lineHeight: 1, margin: "0 0 2px", paddingBottom: 4 }}>{isEN ? FLYER_STRINGS.servicesOffered.en : FLYER_STRINGS.servicesOffered.fr}</h2>
          <ServicesList items={servicesOffered} color={categoryColor} isEN={isEN} />
        </section>
      </div>

      {nearbyServices.length > 0 && <section style={{ marginTop: 7 }}>
        <h2 style={{ borderBottom: `1px solid ${FLYER_COLORS.black}`, color: FLYER_COLORS.black, fontSize: 13, lineHeight: 1, margin: "0 0 2px", paddingBottom: 2 }}>{isEN ? FLYER_STRINGS.alsoOnThisMap.en : FLYER_STRINGS.alsoOnThisMap.fr}</h2>
        {nearbyServices.map((destination) => <AlternativeCentre key={destination.id} destination={destination} isEN={isEN} />)}
      </section>}
    </main>
    <a href="tel:211" aria-label={isEN ? FLYER_STRINGS.call211Aria.en : FLYER_STRINGS.call211Aria.fr} style={{ alignItems: "center", background: FLYER_COLORS.footer, color: "#fff", display: "flex", fontSize: 9.5, fontWeight: 700, gap: 9, justifyContent: "center", marginTop: 7, padding: "8px 16px", textDecoration: "none" }}><Phone size={18} fill="#fff" color="#fff" strokeWidth={1.7} aria-hidden="true" />{isEN ? FLYER_STRINGS.call211.en : FLYER_STRINGS.call211.fr}</a>
  </div>;
}
...
function FlyerMap({ location, service, nearbyServices, isEN }) {
  ...
  return <div aria-label={`Leaflet map from ${location.name} to ${service.name}`} style={{ height: 245, overflow: "hidden", position: "relative" }}>
    ...
    <FlyerMapLegend items={legendItems} isEN={isEN} />
  </div>;
}
...
function FlyerMapLegend({ items, isEN }) {
  return <div className="flyer-map-legend" aria-label="Flyer map legend" style={{ background: "rgba(255,255,255,0.94)", border: "1px solid rgba(17,17,17,0.18)", borderRadius: 3, bottom: 7, color: FLYER_COLORS.black, display: "grid", fontFamily: "Arial, sans-serif", fontSize: 6.6, gap: 2, left: 7, lineHeight: 1.1, padding: "4px 5px", pointerEvents: "none", position: "absolute", zIndex: 1000 }}>
    {items.map((item) => <span key={item.category || "location"} style={{ alignItems: "center", display: "flex", gap: 3 }}><span aria-hidden="true" style={{ background: item.color, border: "1px solid rgba(0,0,0,0.18)", borderRadius: "50%", display: "inline-block", flexShrink: 0, height: 6, width: 6 }} />{isEN ? item.label.en : item.label.fr}</span>)}
  </div>;
}

function getStreetAddress(address, isEN = true) {
  const fallback = isEN ? FLYER_STRINGS.addressNotListed.en : FLYER_STRINGS.addressNotListed.fr;
  return String(address || fallback).split(",")[0].trim();
}
...
function ServicesList({ items, color, isEN = true }) {
  const icons = [UsersRound, BookOpen, BriefcaseBusiness, Heart];
  if (items.length === 0) return <div style={{ color: FLYER_COLORS.lightText, fontSize: 8.5, lineHeight: 1.2, padding: "5px 0" }}>{isEN ? FLYER_STRINGS.contactForDetails.en : FLYER_STRINGS.contactForDetails.fr}</div>;
...
function AlternativeCentre({ destination, isEN = true }) {
